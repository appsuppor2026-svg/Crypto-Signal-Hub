...
import { X, BellOff, ArrowUpRight, ArrowDownRight, CheckCircle2, Bell, BellRing, TrendingUp, TrendingDown, ArrowLeftRight, ArrowUp, ArrowDown } from 'lucide-react';
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="h-[90vh] bg-background border-t border-border flex flex-col sm:max-w-[430px] sm:mx-auto sm:right-auto sm:left-1/2 sm:-translate-x-1/2 rounded-t-xl px-4 pb-6">
          ...
          <SheetTitle className="font-mono text-xl flex items-center gap-2">
            ...
            {activeSmartCount > 0 && (
              <Badge className="bg-primary/20 text-primary border-none text-xs font-mono ml-1">
                {activeSmartCount} activa{activeSmartCount > 1 ? 's' : ''}
              </Badge>
            )}
          </SheetTitle>
        </SheetHeader>
            ...
            <div className="flex-1 min-w-0">
              {notificationPermission === 'denied' ? (
                ...
                <>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Recibe alertas aunque tengas la app en segundo plano.
                  </p>
                  <Button
                    size="sm"
          ...
          <div>
            ...
            {alerts.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-28 text-muted-foreground">
                ...
                <p className="text-sm">No tienes alertas de precio</p>
                <p className="text-xs opacity-60 mt-0.5">Añade una abajo</p>
              </div>
            ) : (
              <div className="space-y-2">
        ...
        <div className="pt-4 border-t border-border space-y-3">
          <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Nueva alerta de precio</p>
