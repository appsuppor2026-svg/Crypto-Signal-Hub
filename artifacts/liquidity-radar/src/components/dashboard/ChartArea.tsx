...
// ── Candle renderer ─────────────────────────────────────────────────────────
// recharts ComposedChart+Line uses scalePoint (not scaleBand).
// scalePoint: xScale(value) returns the CENTER x-position of that point.
// scaleBand:  xScale(value) returns the LEFT edge (add bandwidth/2 for center).
const CandleSticks = ({ xAxisMap, yAxisMap, data }: any) => {
  ...
  return (
    <g>
      {(data as OHLCPoint[]).map((point) => {
        const x0 = xScale(point.date);
        if (x0 == null) return null;

        // scalePoint → x0 is already the centre; scaleBand → x0 is left edge
        const cx: number = isBandScale ? x0 + step / 2 : x0;
...
export function ChartArea({ asset }: ChartAreaProps) {
  ...
  // Initialise with the asset's existing chart data so the line chart is never empty
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (!mounted) return;
      // Only replace if we got real data – keep previous data on failure
  ...
  // Line chart enriched with indicators
  ...
  // Fall back to line mode if candles not yet available
  ...
  return (
    <Card className="bg-[#0a0a0a] border-border rounded-2xl overflow-hidden shadow-xl">
      <CardHeader className="p-3 pb-2 border-b border-border/30 space-y-2">
        ...
        {/* Row 2: indicator toggles – visible in BOTH modes */}
        <div className="flex gap-1.5">
          ...
          <span className="ml-auto text-[9px] text-muted-foreground/40 self-center font-mono">
            {TF_LABELS[selectedTF]} · {displayMode === 'candles' ? `${ohlcData.length} velas` : `${chartData.length} pts`}
          </span>
        </div>
      </CardHeader>
      ...
      <CardContent className="p-0 relative">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={enrichedLine} margin={{ top: 8, right: 0, left: 0, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="price" stroke={strokeColor} strokeWidth={2}
                    fillOpacity={1} fill="url(#cgPriceLR)"
        ...
        {/* ── CANDLE CHART ─────────────────────────────────────────── */}
        {displayMode === 'candles' && (
          <div style={{ height: 315 }} className="w-full pt-3 pr-1">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={ohlcData} margin={{ top: 8, right: 0, left: 0, bottom: 0 }}>
                ...
                {/* EMA overlay on candles when active */}
                {showEMA && (() => {
                  const prices = ohlcData.map(d => d.close);
                  const ema = calculateEMA(prices, Math.min(50, prices.length));
                  const enriched = ohlcData.map((d, i) => ({ ...d, ema: ema[i] ?? null }));
                  return (
                    <Line data={enriched} type="monotone" dataKey="ema"
                      stroke="#f97316" strokeWidth={1.5} dot={false} isAnimationActive={false} connectNulls />
                  );
                })()}

                {/* BB overlay on candles when active */}
                {showBB && (() => {
                  const prices = ohlcData.map(d => d.close);
                  const bb = calculateBollingerBands(prices, Math.min(20, prices.length));
                  const enriched = ohlcData.map((d, i) => ({
                    ...d,
                    bbU: bb[i]?.upper ?? null,
                    bbM: bb[i]?.middle ?? null,
                    bbL: bb[i]?.lower ?? null,
                  }));
                  return <>
                    <Line data={enriched} type="monotone" dataKey="bbU" stroke="#3b82f6" strokeWidth={1} strokeDasharray="3 3" opacity={0.7} dot={false} isAnimationActive={false} connectNulls />
                    <Line data={enriched} type="monotone" dataKey="bbM" stroke="#3b82f6" strokeWidth={1} opacity={0.45} dot={false} isAnimationActive={false} connectNulls />
                    <Line data={enriched} type="monotone" dataKey="bbL" stroke="#3b82f6" strokeWidth={1} strokeDasharray="3 3" opacity={0.7} dot={false} isAnimationActive={false} connectNulls />
                  </>;
                })()}

                {/* Invisible line to anchor Y-scale for Customized renderer */}
            ...
            {/* SQZ sub-panel below candles */}
            {showSQZ && (() => {
              const prices = ohlcData.map(d => d.close);
              const sqz = calculateSqueezeMomentum(prices);
              const enriched = ohlcData.map((d, i) => ({ ...d, sqzMomentum: sqz[i]?.momentum ?? null }));
              return (
                <div className="w-full h-[52px] pb-2 pr-11 -mt-2">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={enriched} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                      <ReferenceLine y={0} stroke="hsl(var(--border))" />
                      <Bar dataKey="sqzMomentum">
                        {enriched.map((e, i) => <Cell key={i} fill={(e.sqzMomentum || 0) > 0 ? '#22c55e' : '#ef4444'} />)}
                      </Bar>
                      <XAxis dataKey="date" hide />
                    </BarChart>
                  </ResponsiveContainer>
